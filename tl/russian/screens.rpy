# TODO: Translation updated at 2026-09-17 18:17

translate russian strings:

    # game/screens.rpy:564
    old "Start"
    new "Начать"

    # game/screens.rpy:568
    old "History"
    new "История"

    # game/screens.rpy:570
    old "Save"
    new "Сохранить"

    # game/screens.rpy:572
    old "Load"
    new "Загрузить"

    # game/screens.rpy:574
    old "Preferences"
    new "Настройки"

    # game/screens.rpy:578
    old "End Replay"
    new "Завершить повтор"

    # game/screens.rpy:582
    old "Main Menu"
    new "Главное меню"

    # game/screens.rpy:584
    old "About"
    new "Об"

    # game/screens.rpy:589
    old "Help"
    new "Помощь"

    # game/screens.rpy:595
    old "Quit"
    new "Выйти"

    # game/screens.rpy:766
    old "Return"
    new "Вернуться"

    # game/screens.rpy:855
    old "{i}{size=32}(You can click the bold text to be linked to their respective page){/size}{/i}"
    new ""

    # game/screens.rpy:861
    old "Story & Writing - {a=https://twitter.com/sigyaad}Sigyaad{/a}"
    new ""

    # game/screens.rpy:863
    old "Artist - {a=https://twitter.com/Rr001C}CRR001{/a}"
    new ""

    # game/screens.rpy:865
    old "Programming Help - {a=https://www.youtube.com/c/AlfredPros/about}AlfredPros{/a}"
    new ""

    # game/screens.rpy:870
    old "Song 1 - {a=https://dova-s.jp/bgm/play14618.html}\"Wind, falling down flower written by Kamaboko Sachiko\"{/a}"
    new ""

    # game/screens.rpy:872
    old "Song 2 - {a=https://www.youtube.com/watch?v=2kWuQz9sdfI}\"Mountain Hare\"{/a}"
    new ""

    # game/screens.rpy:878
    old "Made with {a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only].\n\n[renpy.license!t]"
    new ""

    # game/screens.rpy:929
    old "Page {}"
    new ""

    # game/screens.rpy:929
    old "Automatic saves"
    new "Автоматическое сохранение"

    # game/screens.rpy:929
    old "Quick saves"
    new "Быстрое сохранение"

    # game/screens.rpy:964
    old "{b}{#file_time}%Y/%m/%d  %H : %M : %S{/b}"
    new ""

    # game/screens.rpy:964
    old "{color=#ccc}Empty Save Slot{/color}"
    new ""

    # game/screens.rpy:1356
    old "Keyboard"
    new "Клавиатура"

    # game/screens.rpy:1357
    old "Mouse"
    new "Мышь"

    # game/screens.rpy:1360
    old "Gamepad"
    new ""

    # game/screens.rpy:1373
    old "Enter"
    new ""

    # game/screens.rpy:1374
    old "Advances dialogue and activates the interface."
    new ""

    # game/screens.rpy:1377
    old "Space"
    new ""

    # game/screens.rpy:1378
    old "Advances dialogue without selecting choices."
    new ""

    # game/screens.rpy:1381
    old "Arrow Keys"
    new ""

    # game/screens.rpy:1382
    old "Navigate the interface."
    new ""

    # game/screens.rpy:1385
    old "Escape"
    new ""

    # game/screens.rpy:1386
    old "Accesses the game menu."
    new ""

    # game/screens.rpy:1389
    old "Ctrl"
    new ""

    # game/screens.rpy:1390
    old "Skips dialogue while held down."
    new ""

    # game/screens.rpy:1393
    old "Tab"
    new ""

    # game/screens.rpy:1394
    old "Toggles dialogue skipping."
    new ""

    # game/screens.rpy:1397
    old "Page Up"
    new ""

    # game/screens.rpy:1398
    old "Rolls back to earlier dialogue."
    new ""

    # game/screens.rpy:1401
    old "Page Down"
    new ""

    # game/screens.rpy:1402
    old "Rolls forward to later dialogue."
    new ""

    # game/screens.rpy:1406
    old "Hides the user interface."
    new ""

    # game/screens.rpy:1410
    old "Takes a screenshot."
    new ""

    # game/screens.rpy:1414
    old "Toggles assistive {a=https://www.renpy.org/l/voicing}self-voicing{/a}."
    new ""

    # game/screens.rpy:1420
    old "Left Click"
    new ""

    # game/screens.rpy:1424
    old "Middle Click"
    new ""

    # game/screens.rpy:1428
    old "Right Click"
    new ""

    # game/screens.rpy:1432
    old "Mouse Wheel Up\nClick Rollback Side"
    new ""

    # game/screens.rpy:1436
    old "Mouse Wheel Down"
    new ""

    # game/screens.rpy:1443
    old "Right Trigger\nA/Bottom Button"
    new ""

    # game/screens.rpy:1447
    old "Left Trigger\nLeft Shoulder"
    new ""

    # game/screens.rpy:1451
    old "Right Shoulder"
    new ""

    # game/screens.rpy:1456
    old "D-Pad, Sticks"
    new ""

    # game/screens.rpy:1460
    old "Start, Guide"
    new ""

    # game/screens.rpy:1464
    old "Y/Top Button"
    new ""

    # game/screens.rpy:1467
    old "Calibrate"
    new ""

    # game/screens.rpy:1828
    old "Back"
    new "Назад"

    # game/screens.rpy:1829
    old "Skip"
    new "Пропуск"

    # game/screens.rpy:1830
    old "Auto"
    new "Авто"

    # game/screens.rpy:1831
    old "Menu"
    new "Меню"

